...
@can('orden_compra_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.ordenes_compra.create") }}">
                ...
                <tbody>
                    @foreach($ordenes as $key => $orden)
                        <tr data-entry-id="{{ $orden->id }}">
                            ...
                            <td>
                                @can('orden_compra_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.ordenes_compra.show', $orden->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('orden_compra_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.ordenes_compra.edit', $orden->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('orden_compra_delete')
                                    <form action="{{ route('admin.ordenes_compra.destroy', $orden->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                    </form>
                                @endcan
                                @can('orden_compra_upload')
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Subir imagen de la orden de compra</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
      <form action="{{ route('admin.ordenImageCompra.save') }}" method="POST"  enctype="multipart/form-data">
          @csrf
...
@can('orden_compra_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.ordenes_compra.massDestroy') }}",
